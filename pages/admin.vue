<template>
  <div style="margin-top: 150px; padding: 10px">
    <div v-if="!passed" style="display: flex; justify-content: center; align-items: center">
      <v-card width="500px" elevation="10">
        <v-card-title>Admin</v-card-title>

        <v-card-text>
          <v-otp-input v-model="password" length="8"></v-otp-input>
        </v-card-text>
      </v-card>
    </div>
      ...
      <v-list>
        ...
        >
          <v-list-item-content>
            ...
            <v-list-item-subtitle>{{ user.email }} / {{ user.phone }}</v-list-item-subtitle>
          ...
          <v-list-item-action>
            <v-btn variant="tonal" class="mt-3" @click="acceptUser(user.id)">Accept</v-btn>
          </v-list-item-action>
        </v-list-item>
      </v-list>
    </div>
